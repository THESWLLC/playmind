...
| Screen capture | `PYTHONPATH=. python scripts/capture_once.py --ocr` |
| Owned-game loop (dry-run) | copy config → set `i_own_this_game` → `scripts/run_owned_loop.py` |
| Owned play+learn (live, admin) | `scripts\PLAY_AS_ADMIN.bat` → Ollama `llama3.2` + learning |
...
3. Calibrate capture (window title or monitor) / `hp_roi` (`[left, top, right, bottom]`)
   ```bash
   PYTHONPATH=. python scripts/capture_once.py --list-windows
   PYTHONPATH=. python scripts/capture_once.py --window Ascension
   ```
4. Edit keymap
5. Dry-run first:
   ```bash
   PYTHONPATH=. python scripts/run_owned_loop.py --config config/owned_game.json --max-ticks 10
   ```
6. Live keys (optional):
   ```bash
   pip install pynput mss Pillow pytesseract
   # set enable_keyboard true
   PYTHONPATH=. python scripts/run_owned_loop.py --config config/owned_game.json --live --max-ticks 10
   ```
## Local LLM
